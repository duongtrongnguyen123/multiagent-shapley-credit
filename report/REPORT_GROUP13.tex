...
A second control at the same three-call budget removes the role labels altogether. In the
\texttt{SS\_anc} configuration the solver is called three times; the second and third calls are
told only that ``a previous attempt answered $X$'', with no instruction to check anything. The
word \emph{check} is deliberately absent, so what is removed relative to \texttt{PSV} is the
verification framing while the flow of information between passes is preserved.

\begin{table}[h]\centering\small
\begin{tabular}{@{}l c c c c c@{}}
\toprule
 & One generation & \texttt{PSV} & \texttt{SS\_anc} & Difference & Tokens \texttt{PSV} / \texttt{SS\_anc}\\
\midrule
GSM8K 1.5B & 0.632 & 0.728 & \textbf{0.728} & $0.0$ (2/5) & 116k / 169k\\
MATH 1.5B  & 0.330 & 0.380 & 0.360 & $-2.0$ (3/5) & 197k / 248k\\
GSM8K 7B   & 0.912 & 0.904 & 0.924 & $+2.0$ (2/5) & 115k / 179k\\
MATH 7B    & 0.500 & 0.590 & 0.480 & $\mathbf{-11.0}$ (0/5) & 199k / 255k\\
\bottomrule
\end{tabular}
\caption{Three sequential solver calls with an anchor (\texttt{SS\_anc}) against the three-role
pipeline (\texttt{PSV}) at the same call count. Accuracy on a 0--1 scale; the difference column is
$\texttt{SS\_anc}-\texttt{PSV}$ in points, with the number of folds out of five in which
\texttt{SS\_anc} scored higher. Token counts are generated tokens per arm.}
\label{tab:anchor}
\end{table}

In three of the four cells the labels buy nothing measurable. On GSM8K at 1.5B the two
configurations land on the same number to three decimals. The other two differences are $2.0$
points with fold splits of 2/5 and 3/5, below the effective threshold of \S\ref{sec:setup} and
without a consistent direction.

The fourth cell does not follow. On MATH at 7B the named pipeline is ahead by $11.0$ points and
\texttt{SS\_anc} loses in all five folds. On the one cell where the model is capable and the task
is hard, presenting the third pass as verification does something that presenting it as another
attempt does not; we did not isolate what.

Two readings therefore have to be held together. The role labels do not account for the accuracy
at three of four operating points, which is consistent with the behavioural evidence in
\S\ref{sec:roles}--\ref{sec:aggregatorrole}. But \texttt{PSV} is also never worse and is cheaper
in every cell, by 25\% to 56\% of generated tokens, because a short plan and a critique cost less
than three full solutions. The arrangement can be an efficient way to spend three calls even
where the components are not performing the functions their names describe.
