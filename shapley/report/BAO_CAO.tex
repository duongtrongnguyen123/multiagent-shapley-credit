...
% ---------------------------------------------------------------
\subsection{Điểm xuất phát: đánh giá lợi ích của pipeline và mức dao động nền}\label{sec:start}
...
\bottomrule
\end{tabular}
\caption{So sánh pipeline đầy đủ bốn vai với một lượt giải duy nhất (Qwen-1.5B; đo một lần trên
...
% ---------------------------------------------------------------
\subsection{Kiểm soát chi phí: giá trị nằm ở lượt sinh bổ sung, không phải ở vai tổng hợp}\label{sec:budget}
...
\bottomrule
\end{tabular}
\caption{So sánh các bộ tổng hợp với cùng ngân sách 8 lượt sinh (hàng trên cùng là mốc 1 lượt để tham chiếu). Các ô là độ chính xác, thang 0--1.}
\label{tab:budget}
\end{table}
...
\emph{Thứ ba}, một đối chứng tách được ảnh hưởng của ``phối hợp'' khỏi ảnh hưởng của ``thêm lượt sinh''. Khi cho giải lại mà \emph{không} xem phê bình của verifier, kết quả bằng đúng giải lại \emph{có} xem phê bình ($\texttt{rerun} = \texttt{loop}$, cùng cho $0{,}453$). Lợi ích vì vậy đến từ lượt sinh thêm, không từ nội dung phản hồi. (Một phép đo đơn lẻ trước đó cho $+20$ điểm nhưng không tái lập: trên 5 fold chỉ còn $+4{,}0$ điểm.) Nhất quán với điều đó, biến thể vòng lặp giải-rồi-chấm --- rẻ hơn pipeline đủ vai --- còn thắng nó trên MATH: $+4{,}0$ điểm (KTC $[+0{,}5;+7{,}5]$; $t=3{,}21$) ở cùng chi phí.
...
\caption{Sơ đồ pipeline bốn vai, 16 tổ hợp bật/tắt để tính giá trị Shapley, và quy trình ước lượng nhiễu qua 5 fold.}
\label{fig:shapleyfw}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Phân tầng theo số lượt đúng trong 5 lượt sinh (MATH 1.5B, $n=150$). Cột $\Delta$ là chênh lệch độ chính xác giữa \texttt{maj@5} và Solver.}
\label{tab:strata}
\end{table}
...
\emph{Hết mạch thứ nhất.} Bức tranh sau ba phép kiểm soát: giá trị dương chắc chắn duy nhất là lượt sinh thêm; cấu hình sửa-bài-của-nhau thua model mạnh đơn lẻ. Hồi II xác định giá trị mất đi đâu và có quy luật nào đoán trước được hay không.
...
Trên HumanEval, cùng model, cùng 4 lượt sinh, chỉ khác nhau ở nguồn kiểm tra, kết quả được trình bày trong Bảng~\ref{tab:exec}.
...
Hai kết quả về “chọn lúc nào cần gọi thêm” đều ở mức B (đo một lần, $n=300$). Bộ định tuyến đồng thuận (chạy $S$ và $V$, chỉ gọi $A$ khi hai vai bất đồng) trên GSM8K giữ gần trọn phần lợi của pipeline với 77\% chi phí (0,720 so với 0,723 ở 2,32 so với 3 lượt); trên MATH đạt 0,413, bằng với solver đơn lẻ và không mang lại lợi ích. Cơ chế phía sau nhất quán với \S\ref{sec:termL}: tỷ lệ bài model yếu sai (tức tỷ lệ artifact sai trong ngữ cảnh) khoảng 33\% trên GSM8K so với 59\% trên MATH; khi hai vai bất đồng, $A$ cứu được 45,4\% trường hợp trên GSM8K nhưng chỉ 25,0\% trên MATH. Do đó, quan sát “định tuyến tiết kiệm ở đúng nơi ít cần tiết kiệm nhất” chỉ dừng ở mức B.
...
Nhóm đã thử bảy phương pháp huấn luyện/tối ưu vai (Bảng~\ref{tab:rl}). Không phương pháp nào đạt chuẩn dự án (hiệu ứng vượt ngưỡng và 5/5 fold cùng dấu) trên tập lạ, và mỗi thất bại đều truy được về việc model tìm ra lối tắt của hàm mục tiêu thay vì học năng lực đích.
...
\emph{thứ năm}: Credit-RL (thưởng bằng phần đóng góp Shapley biên) không giúp vai nào cải thiện theo chuẩn 5/5 fold. Planner sập về plan rỗng trong 200/200 câu; verifier có 23 lần sửa và 24 lần phá.
