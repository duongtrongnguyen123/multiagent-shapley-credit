...
% Bản v1.0 — biên dịch: pdflatex (hoặc Overleaf, mặc định)
...
Sau ba kiểm soát, bức tranh đảo chiều có hệ thống. Hiệu ứng dương lớn nhất ($+14{,}0$ điểm, 5/5
fold) chỉ tồn tại khi so với model yếu; so cùng chi phí FLOP với model mạnh đơn lẻ thì hoà hoặc
kém. Nguyên nhân được định vị bằng một đẳng thức đại số $\dceil = A - B + C$ (khớp tuyệt đối
19/19 cặp đo) và một thí nghiệm tiền đăng ký tái lập trên hai miền: thiệt hại đến từ việc model
mạnh \textbf{tiếp xúc với nội dung sai} --- trên MATH, độ chính xác của nó rơi từ 46,4\% xuống
19,2\% trên tầng artifact sai ($-27{,}2$ điểm; $p \approx 0$), trong khi artifact đúng làm nó
\emph{khá lên} ($+3{,}8$ điểm; $p = 0{,}012$). Cùng một biến chênh lệch năng lực nuôi cả cơ hội
...
càng thua --- nghịch lý biểu kiến được giải trọn.
...
$\texttt{oracle@}k - \texttt{maj@}k$, không bởi chất lượng tín hiệu. Khuyến nghị thực tiễn:
\emph{dùng model nhỏ để giải, model lớn để soát; sinh độc lập rồi chọn; đừng cho model mạnh xem
bài làm của model yếu với mục đích sửa.}
\end{abstract}

% =====================================================================
\section{Mở đầu}\label{sec:intro}

Ghép nhiều vai LLM thành một hệ có tạo thêm giá trị so với một model đơn lẻ không? Và nếu có,
giá trị đến từ đâu --- từ \emph{cấu trúc phối hợp}, hay từ thứ khác đi kèm cấu trúc đó?

Câu hỏi khó trả lời hơn vẻ ngoài, vì ba vấn đề đo lường.
...
$3\times$ với một hệ $1\times$ rồi kết luận ``phối hợp có giá trị'' là so sánh khập khiễng.
...
model mạnh trong pipeline, là: chạy pipeline, hay \textbf{gọi thẳng model mạnh} ($V-I$)? Khảo
sát này cho thấy hai mốc đó cho \textbf{hai dấu ngược nhau} trên cùng một hệ
...
\textbf{mạch dẫn dắt}: mỗi thí nghiệm sinh ra từ câu hỏi mà thí nghiệm trước để lại.
...
\begin{enumerate}\itemsep2pt
  ...
  \item Bằng chứng trên hai thiết kế độc lập rằng \textbf{chênh lệch năng lực} --- không phải
        số lượng vai, danh tính vai, hay họ model --- là biến quyết định (\S\ref{sec:roles},
        \S\ref{sec:termA}).
  \item Một \textbf{đẳng thức đại số} $\dceil = A - B + C$ tách cơ hội khỏi thiệt hại, khớp
        tuyệt đối trên 19/19 cặp đo (\S\ref{sec:framework}, \S\ref{sec:termA}).
  \item Bằng chứng tiền đăng ký, tái lập hai miền: thiệt hại của giao thức sửa đến từ
        \textbf{tiếp xúc với nội dung sai}, không phải từ tiếp xúc; và giao thức sửa là
        \textbf{ống dẫn, không phải bộ sửa lỗi} --- truyền đáp án đúng gần trọn vẹn (90,9\%)
  ...
  \item Một bộ quy trình chống tự đánh lừa (tiền đăng ký, điều kiện hợp lệ, niêm phong hash,
        sàn nhiễu, t-test theo fold) mà \textbf{50\% số lần chạy không vượt qua} --- bản thân
        tỷ lệ đó là một kết quả (\S\ref{sec:method}).
\end{enumerate}
...
giải mới ở bước tổng hợp}, tức $B \approx 0$ theo thuật ngữ của chúng tôi.
...
kết quả \texttt{llm3} và phân tầng tiếp xúc của chúng tôi. \textbf{Điểm chúng tôi nhấn mạnh: các
công trình dòng này chủ yếu báo cáo cải thiện so với model bị sửa hoặc lượt nháp đầu ($V-S$),
không so với việc dùng thẳng model sửa ($V-I$)} \todoD{rà từng bài, xác nhận mốc so sánh của
...
\todoD{nguồn cụ thể} cho thấy tranh biện thua self-consistency ở đa số ô so sánh và \textbf{sụp
16 điểm} với Llama-3.1-8B trên GSM8K ($80{,}13 \to 63{,}87$) --- khớp với mẫu hình sụp đổ ở
...
suy diễn bằng t-test trên fold ($\mathrm{SE} = s/\sqrt{k}$; $t_{0{,}975}(4) = 2{,}776$), KTC
95\%, và phép thử dấu (5/5 cùng dấu $\Rightarrow p = 0{,}031$ một phía). Khối luồng thông tin:
...
\emph{Trình bày theo mạch: mỗi mục mở bằng câu hỏi kế thừa, đóng bằng câu hỏi để lại. Số liệu
mức A trừ khi ghi khác.}
...
Trên GSM8K có lợi (5 fold: $\texttt{PSVA}-\texttt{PS} = +5{,}6$; KTC $[+3{,}3;\,+7{,}9]$;
$t=6{,}89$); trên MATH \textbf{tốn 6,63 lần để kém hơn 6 điểm}. Phép hiệu chuẩn sàn nhiễu ---
...
\emph{Câu hỏi để lại: phần lợi có phải do phối hợp, hay chỉ do được sinh nhiều lượt?}
...
phá đa số đúng 21/26 lần, sửa được đa số sai 2/0 lần. Kết quả \textbf{phụ thuộc task}
($\texttt{vote5}-\texttt{llm\_agg}$: $+0{,}075$ MATH, $-0{,}056$ GSM8K) --- phát biểu đúng:
\emph{bộ tổng hợp LLM không mang giá trị ổn định; ở miền khó nó gây hại đáng kể.}
...
\item \textbf{\texttt{rerun} = \texttt{loop}} ($0{,}453 = 0{,}453$): giải lại \emph{không xem}
      ...
      từ \textbf{lượt sinh thêm}, không từ nội dung phản hồi. (Con số $+20$ đo một lần không tái
      lập: 5 fold còn $+4{,}0$.) Biến thể vòng lặp giải-rồi-chấm rẻ hơn pipeline đủ vai lại
      \textbf{thắng nó} trên MATH: $\texttt{loop}-\texttt{PSVA} = +4{,}0$; KTC $[+0{,}5;\,+7{,}5]$;
      $t = 3{,}21$ --- cùng chi phí (4,2 so với 4,0 lượt).
\item \textbf{Aggregator không chọn --- nó chép.} 65\% số câu nó chép nguyên ứng viên
...
\emph{Câu hỏi để lại: mọi can thiệp đều cho 0--5 điểm --- có phải đang đo sai mẫu số?}
...
\textbf{57\% số câu không thể có hiệu ứng vì lý do toán học.} Trên tầng khả tác động: $+21{,}5$
điểm (nhóm 1--4/5, cách nhóm của tài liệu nguồn) hay $+31{,}1$ (nhóm 2--4/5, loại tầng hiệu ứng
bằng 0); trung bình toàn tập pha loãng còn $+9{,}3$. Một can thiệp $+10$ điểm thật sẽ hiện ra
$+3$ --- dưới sàn nhiễu, và bị kết tội oan là vô dụng. Ba kiểm soát tác động \textbf{ngược
chiều}: thiếu mẫu số $\Rightarrow$ đánh giá thấp; thiếu ngân sách/mốc $\Rightarrow$ đánh giá cao.

\emph{Câu hỏi để lại: trong phần khả tác động, vai nào đóng góp?}

% ---------------------------------------------------------------
\subsection{Vai trò: chuyên biệt hoá sụp đổ; biến thật là năng lực của lượt kiểm}\label{sec:roles}
...
dài \textbf{19 ký tự} khi có plan (664 khi không). \emph{Nó không giải --- nó chép.} Ở 7B, hai
...
$[-2{,}7;\,+13{,}4]$ --- không đạt. \textbf{Danh tính vai không phải biến.}

Biến thật là \textbf{năng lực của model ở lượt thứ hai}. Cùng 300 bài MATH (5 fold $\times$ 60):
...
Riêng phần do lớn hơn & $\mathbf{+11{,}0}$ & $[+3{,}9;\,+18{,}1]$ & ---\\\bottomrule
\end{tabular}
\caption{Bất đối xứng năng lực ở lượt kiểm, MATH.}
\end{table}

Cơ chế từ trace: khi can thiệp, verifier \textbf{tái sử dụng 0\%} số liệu của solver --- nó
không dò từng bước, nó \textbf{giải lại từ đầu}. \emph{Verifier là một solver thứ hai đội lốt bộ
kiểm; mua verifier 7B là mua một solver tốt hơn cho lượt thứ hai.} Phù hợp: \texttt{V\_gain} có
ý nghĩa ở cả MATH 7B ($+4{,}4$; KTC $[+1{,}5;\,+7{,}3]$) --- giá trị verifier xuất hiện
\emph{cùng} năng lực; ở MATH 1.5B thì không ($+1{,}4$; $p=0{,}235$).

\emph{Câu hỏi để lại: $+14{,}0$ là so với model yếu. So đúng mốc thì sao?}
...
Kết luận: \textbf{trên GSM8K cấu hình bất đối xứng bị chi phối hoàn toàn} (kém 10 điểm, không rẻ
hơn); trên MATH là tiết kiệm \emph{biên} với độ chính xác hoà. Solver đơn lẻ nằm trên đường
Pareto ở cả hai task; pipeline đầy đủ không nằm trên đường Pareto ở MATH. Cải thiện thật duy
nhất trong bảng: thêm V7B vào \emph{chính} S7B trên MATH: $+7{,}7$ ($p = 0{,}0075$) --- với giá
$1{,}7\times$ token.

Đây là dạng tổng quát của vấn đề mốc: $V-S$ dương, $V-I$ âm --- \textbf{cùng hệ, hai mốc, hai
kết luận ngược nhau}, lặp lại ở mọi nhánh của khảo sát.

\emph{Câu hỏi để lại: giá trị mất đi đâu? (Từ đây dùng đẳng thức \S\ref{sec:identity}.)}
...
\textbf{đúng bằng solver đơn lẻ}, vô dụng. Cơ chế: khi $S$--$V$ bất đồng, $A$ cứu 45,4\% trường
...
\end{align*}
$R^2$ chỉ với chênh lệch $= 0{,}824$. KTC của $\beta_2$ nằm \textbf{trọn dưới} ngưỡng $+0{,}02$
đã khoá trước --- \textbf{null có thông tin}: ``khác họ'' là tương quan giả; biến thật là chênh
lệch năng lực. (Kết quả đa dạng pool --- 3 model cho 2,70/3 ứng viên phân biệt so với 1,91/3 khi
...
g^\ast \approx 0{,}09
\end{equation}
Đẳng thức $A-B+C$ khớp 15/15. Đọc kèm bắt buộc: \textbf{0/15 cặp dương có ý nghĩa; 3/15 âm có ý
nghĩa} $\Rightarrow$ chỉ phát biểu \textbf{chiều phủ định}: \emph{chênh lệch vượt $\sim$0,09 thì
đừng dùng giao thức sửa.} Xác lập vùng dương cần $\sim$8$\times$ toàn bộ MBPP --- bất khả thi
...
đôi khả năng bảo toàn thực của nó, hệ số 1,10; chênh lệch hệ số $p = 0{,}0066$. Giao thức sửa
chết vì số học ngân sách, không phải vì model mạnh ``yếu bóng vía''.)}

\emph{Câu hỏi để lại: $B$ do đâu mà có?}
...
\end{tabular}
\caption{Hiệu ứng của việc nhìn thấy artifact, phân tầng theo nội dung. Kết quả trung tâm.}
\end{table}

Trên tầng artifact sai, model mạnh rơi \textbf{46,4\% $\to$ 19,2\%} --- trên chính những bài nó
vốn giải được gần một nửa; artifact đúng làm nó khá lên. Tổng hợp trọng số tái tạo \emph{chính
xác} $V-I = -0{,}1240$. \textbf{$D$ không phải hình phạt của việc nhìn thấy --- mà của việc nhìn
thấy nội dung sai}; và vì thiết kế không có lệnh sửa, \emph{riêng nhìn thấy đã đủ gây hại}.
...
của $S$ gần trọn vẹn (90,9\%), gần như không sáng tạo lời giải mới (4,3\%), phá ở quy mô lớn
(63,6\%); (2) tầng khai thác được nhỏ hơn tầng rủi ro \textbf{11 lần} (11 so với 121 bài); (3)
``giữ nguyên $S$ khi $S$ đúng'' chính là oracle $\CEIL$ --- và tặng không oracle đó cũng chỉ
thêm $+0{,}004$ trên MATH (đúng 2 bài/500), vì $V$ vốn đã giữ được 99,6\%/90,9\%; kể cả vậy
$\CEIL = 0{,}578$ vẫn thua $I = 0{,}698$. \textbf{Không cấu hình nào có $V > I$} trong toàn bộ
khảo sát (0/15 cặp MBPP; tốt nhất $-0{,}030$). Vấn đề không phải ``quên giữ $S$'' --- mà là $V$
phá tầng ($S$ sai, $I$ đúng), và cách duy nhất bảo vệ tầng đó là \emph{đừng cho xem}.
...
viết ``đã xác nhận'': KTC rộng 0,064--0,084 nên phép kiểm có độ phân giải thấp). Cặp lệch lệch
\textbf{hệ thống} --- $B = 0{,}208$, gấp mười lần $A$ --- và tái lập một phép đo độc lập trên
cùng cặp ($-0{,}1380 \to -0{,}1660$): quy luật mô tả xu hướng; ngoại lệ ở nơi model yếu quá yếu
so với bài.

% ---------------------------------------------------------------
\subsection{$\kappa$: đúng đắn đạt trần oracle; học được thì ``đo được $\neq$ dùng được''}\label{sec:kappa}
...
\texttt{exec3} \textbf{phá 0 bài trong 20/20 fold} và đạt đúng \texttt{oracle@4} ở hai ô có ghi
số; \texttt{llm3} phá ở cả 20 fold. Mốc trung thực là greedy --- bỏ phiếu \textbf{có hại trên
...
thi chính xác 0,837 vẫn \textbf{lỗ ròng} (26 phá / 6 sửa).
...
\begin{table}[h]\centering\small
\begin{tabular}{lcccc}\toprule
Model & Lỗi tiêm & Lỗi thật & AUC & Đổi thành độ chính xác\\\midrule
1.5B & $-0{,}012$ & $+0{,}195$ & 0,528 & $-0{,}008$ (1/5 fold)\\
\textbf{7B} & $\mathbf{+0{,}573}$ & $\mathbf{+0{,}693}$ & \textbf{0,893} & $\mathbf{+0{,}024}$ (2/5 fold)\\\bottomrule
\end{tabular}
\caption{Bộ phân loại học được: đo được rất tốt, gần như không dùng được.}
\end{table}

Ở 7B nó học rất tốt, chuyển giao còn tốt hơn trong phân phối --- nhưng AUC 0,893 chỉ mua được
$+2{,}4$ điểm. \textbf{Đo được $\neq$ dùng được.} Phát biểu hợp nhất, nối bốn thí nghiệm:
...
khoảng cách nhỏ --- AUC 0,893 lấy $+2{,}4$. \textbf{Nút thắt nằm ở khâu SINH, không phải khâu
...
tín hiệu vừa chứng minh là đo được mà không dùng được. Kết luận thực tiễn: \textbf{không phải
``làm cổng tốt hơn'' --- mà ``mặc định đừng cho xem''.}
...
\end{align*}
Chênh lệch năng lực nuôi \textbf{cả $A$ lẫn $B$}; giao thức quyết định gặt bên nào. Sáu mảnh
bằng chứng độc lập hội tụ: (1) đại số --- chỉ-chọn đặt $B \approx 0$ theo cấu trúc; (2) độ lớn
...
trên nền $-0{,}124$; (5) ngoại chứng --- tranh biện thua self-consistency và sụp ở model nhỏ
trong số liệu công bố \todoD{nguồn}; (6) liên miền --- quy luật MBPP dự báo được MATH ở 2/3 cặp.
...
\item \textbf{Mọi miền:} đừng cho model mạnh xem bài của model yếu với mục đích sửa --- nhất là
      khi chênh lệch vượt $\sim$0,09; đã có model mạnh thì gọi thẳng nó.
\end{itemize}

\begin{quote}\centering
\textbf{Một câu:} dùng model nhỏ để giải, model lớn để soát;\\
sinh độc lập rồi chọn --- đừng đưa cho nhau sửa.
\end{quote}
...
số; VOID thì không đọc). Các con số tự nói:
\begin{itemize}\itemsep2pt
...
\item \textbf{Rà lại thống kê giữa kỳ:} ngưỡng ``5 điểm'' gốc suy cho phép đo đơn lẻ; với 5
      ...
      $P(\text{hiệu ứng} > 0)$. Rà 131 đại lượng: \textbf{bốn kết quả chủ lực đều đứng vững}
      ($t = 3{,}3$--$6{,}9$; $P(>0) = 98{,}5$--$99{,}9\%$); 6 kết quả được phục hồi \emph{sau
      ...
      tương ứng.
\end{itemize}
Phần lớn ``cải thiện'' ghi nhận ban đầu không sống sót qua kiểm chứng --- đó là một kết quả,
...
``Đa tác tử có giúp không'' không có câu trả lời đơn --- nhưng có câu trả lời \textbf{có cấu
trúc}. Dưới ba phép kiểm soát, giá trị của phối hợp phân rã thành ba thừa số đo được: dư địa
...
và giao thức sửa, khi nhìn kỹ, là một ống dẫn đáp án chứ không phải một bộ sửa lỗi. Nơi có bộ
kiểm đúng đắn, phối hợp chạm trần oracle mà không phá một bài nào; nơi không có, cách khôn
ngoan là sinh độc lập, bỏ phiếu, và đừng để các model đọc bài của nhau.
