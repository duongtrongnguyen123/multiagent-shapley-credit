...
%   sửa ba mâu thuẫn nội dung (đổi dấu theo mốc so sánh, E>I tại chênh 0, chặn trên kappa).
...
% Tên môn học: font sans-serif, cỡ lớn nhất trang
{\sffamily\bfseries\fontsize{26}{31}\selectfont XỬ LÝ NGÔN NGỮ TỰ NHIÊN\par}
\vspace{24pt}

{\sffamily\bfseries\fontsize{15}{18}\selectfont BÁO CÁO BÀI TẬP LỚN CUỐI KỲ\par}
\vspace{16pt}

{\fontsize{13.5}{17}\selectfont Khi nào phối hợp đa tác tử LLM tạo ra giá trị?\par}
{\fontsize{13.5}{17}\selectfont Một khảo sát thực nghiệm dưới ba phép kiểm soát\par}
\vspace{42pt}

% Thành viên: hai cột căn giữa, mã sinh viên căn thẳng dưới tên
{\normalsize
...
Nhóm chúng em xin gửi lời cảm ơn chân thành và sâu sắc nhất tới thầy \textbf{Trần Hồng Việt}, giảng viên phụ trách môn học Xử lý ngôn ngữ tự nhiên, Trường Đại học Công nghệ --- ĐHQGHN.
...
\vspace{1.8cm}
\begin{flushright}
\textit{Hà Nội, tháng 08 năm 2026}\\[6pt]
\textbf{Nhóm sinh viên thực hiện}\\[8pt]
\begin{tabular}{rl}
...
\begin{tikzpicture}[
  ...
  \node[box] (ri) at (-3.6,-5.2) {Đúng \textbf{46,4\%}};
  
  % Nhánh E (chia làm 2 ô)
  \node[box, fill=red!6] (e1) at (3.6,-1.8) {\textbf{Nhánh $E$} \\Model yếu giải};
  \node[box, fill=red!6] (e2) at (3.6,-3.6) {Chèn lời giải sai của model yếu\\vào context của Model mạnh};
  \node[box, fill=red!6] (re) at (3.6,-5.2) {Đúng \textbf{19,2\%}};
...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{Hai mốc so sánh cho hai kết luận ngược nhau}: so với model yếu, hiệu ứng tăng theo chênh lệch năng lực gần như theo định nghĩa; so với model mạnh chạy một mình --- mốc mà người triển khai thực sự đối mặt --- hiệu ứng giảm và đổi dấu. Cùng một hệ, hai mốc, hai kết luận ngược nhau; mốc phổ biến trong tài liệu là mốc bị thổi phồng (\S\ref{sec:synthesis}).
...
\end{enumerate}
Các kết quả được thu thập theo một quy trình kiểm soát độ tin cậy, gồm chốt tiêu chí trước khi
chạy và loại những lần chạy không đạt điều kiện hợp lệ; quy trình này được mô tả ở
\S\ref{sec:setup}. Trong tổng số 32 lần chạy, 16 lần (50\%) bị loại trước khi đọc kết quả. Báo cáo dùng một số ký hiệu riêng; Bảng~\ref{tab:cheatsheet} là chỗ tra nhanh ---
...
\multicolumn{2}{@{}l}{\textbf{Tra nhanh ký hiệu} (chi tiết: \S\ref{sec:framework}--\ref{sec:setup})}\\\midrule
...
artifact & Bài làm của model yếu (nhiễu đưa vào context)\\
$G, L, R$ & Cơ hội ($G$), thiệt hại ($L$), cứu được ($R$)\\
$\dceil$ & Trần lý tưởng $\dceil = G-L+R$\\
$\dhonest$ & Hiệu quả thực tế triển khai: $\acc(E) - \acc(I)$\\
chênh; $g^\ast$ & Hiệu accuracy (mạnh $-$ yếu); điểm đổi dấu $\dceil$ ($\approx 0{,}09$)\\
lần sinh & Một lần model sinh trọn một lời giải; $k$ lần sinh thì độc lập và bình đẳng\\
\texttt{maj@}k; \texttt{oracle@}k & Bỏ phiếu đa số; Trần lý tưởng ($k$ lần sinh)\\
\texttt{exec3} / \texttt{llm3} & Chọn ứng viên bằng: Chạy test / LLM tự duyệt\\
\texttt{V\_gain}, \texttt{A\_gain} & Accuracy thay đổi khi thêm vai trò $V$ hoặc $A$\\
điểm; KTC & Điểm phần trăm ($+14{,}0$ điểm $=+0{,}140$); Khoảng tin cậy 95\%\\
mức dao động nền & Biên chênh lệch khi đo lại cùng cấu hình (tới 7 điểm/100 bài)\\
fold; Mức A / B & 1/5 phần dữ liệu; Kiểm chứng 5-fold ($t$-test) / Đo 1 lần minh hoạ\\\bottomrule
\end{tabular}
\caption{Bảng tra nhanh ký hiệu và thuật ngữ trong báo cáo.}
...
\emph{Hết mạch thứ nhất.} Bức tranh sau ba phép kiểm soát: giá trị dương chắc chắn duy nhất là lần sinh thêm; cấu hình sửa-bài-của-nhau thua model mạnh đơn lẻ. Mạch thứ hai xác định giá trị mất đi đâu và có quy luật nào đoán trước được hay không.

% ---------------------------------------------------------------
\subsection{Chênh lệch năng lực thu hẹp cơ hội cải thiện}\label{sec:termG}

Quan sát thô trên 7 cặp model gộp nhiều lần chạy cho thấy: các cặp khác họ có cơ hội $G$ cao hơn khoảng 24\% (0,0597 so với 0,0481), nhưng các cặp khác họ cũng có chênh lệch năng lực nhỏ hơn (0,130 so với 0,167); hai biến này bị trộn hoàn toàn. Thiết kế tách biệt (chốt trước) sử dụng 6 model, 15 cặp có hướng, trên cùng 499 bài MBPP, hồi quy
\begin{align*}
...
Thêm nhánh sửa cho cả 15 cặp (chốt trước) cho quy luật của trần:
...
Chúng tôi sử dụng đường khớp~\eqref{eq:rule} ước lượng trên MBPP để dự báo $\dceil$ trên MATH (chốt trước; 3 cặp Qwen; KTC bootstrap ghép cặp 10\,000 lần). Kết quả được trình bày trong Bảng~\ref{tab:transfer}.
...
\bottomrule
\end{tabular}
\caption{Hiệu ứng tiếp xúc $\acc(E)-\acc(I)$ (đơn vị: điểm phần trăm), phân tầng theo nội dung artifact. Hai cột MBPP là phân tầng hậu nghiệm (mức B); cột MATH là thí nghiệm chốt trước (mức A).}
\label{tab:exposure}
\end{table}
...
exec3 không phá hủy bài nào trong cả 20/20 fold và đạt đúng \texttt{oracle@4} ở hai ô có ghi số (0,6438 và 0,8812). Ngược lại, llm3 phá hủy ở cả 20/20 fold. Mốc trung thực là greedy; bỏ phiếu đa số có hại trên code ($-11{,}3$ và $-13{,}1$ điểm) vì lời giải dài hiếm khi trùng nhau nguyên văn. Chênh lệch $exec3-\text{greedy}$ dao động từ $+6{,}3$ đến $+10{,}6$ điểm qua bốn điều kiện. Một điều kiện áp dụng quan trọng: khi model nền bão hoà (GSM8K 7B, solver 0,916), một bộ kiểm thực thi có độ chính xác 0,837 vẫn cho hiệu ứng ròng âm (26 phá / 6 sửa). Do đó, “có bộ kiểm chắc chắn” chỉ có giá trị khi còn tiềm năng cải thiện.
...
giữa lời giải đúng và sai, đo trên lỗi tiêm và lỗi thật. Ở 1.5B, AUC $0{,}528 \approx$ đoán ngẫu nhiên: thay một chữ số trong
...
\begin{quote}
Khoảng cách $\texttt{oracle@}k - \texttt{maj@}k$ là chặn trên cho giá trị của mọi bộ kiểm trên cùng pool. Tín hiệu chắc chắn (chạy test) lấy gần trọn chặn đó; tín hiệu học được trong khảo sát của chúng tôi chỉ lấy được một phần nhỏ, dù chất lượng đo lỗi rất cao. Nút thắt của miền toán vì vậy mang tính kép: vừa thiếu tín hiệu chắc chắn, vừa nghèo pool. Trong số bài mà \texttt{maj@5} sai, 65\% (48/74, tính từ Bảng~\ref{tab:strata}) là bài pool không chứa nổi một ứng viên đúng, tức không bộ chọn nào có thể cứu được.
\end{quote}
...
Nhóm đã thử bảy phương pháp huấn luyện/tối ưu vai trò. Không phương pháp nào đạt chuẩn dự án (hiệu ứng vượt ngưỡng và 5/5 fold cùng dấu) trên tập lạ, và mỗi thất bại đều truy được về việc model tìm ra lối tắt của hàm mục tiêu thay vì học năng lực đích.
...
\item[\textbf{Thứ hai.}] GRPO theo đáp án cuối (vá lỗ im lặng) đạt $+1{,}8$ điểm, thấp hơn ngưỡng $+2{,}0$ đã đăng ký trước. Lối tắt là nhại solver: trung vị độ dài đầu ra giảm từ $480$ xuống $19$ ký tự, tỷ lệ đồng ý khi solver sai tăng từ $0{,}50$ lên $0{,}64$.

\item[\textbf{Thứ ba.}] GRPO + bất đối xứng thông tin (solver 0,5B, verifier 1,5B; chốt trước, đủ nhánh $I$) đo đồng thời hai mốc: so với model yếu cho $+17{,}0$ điểm (5/5 fold dương); so với chính model kiểm khi tự giải cho $-10{,}4$ điểm (5/5 fold âm). RL chỉ gỡ được 38\% thiệt hại tiếp xúc; 54\% bài hỏng sinh ra đáp án thứ ba không theo ai.

\item[\textbf{Thứ tư.}] ORPO aggregator (học từ cặp ưa thích) đạt $+3{,}3$ điểm trên MATH (3/5 fold, dưới mức dao động nền) và $-2{,}7$ điểm trên GSM8K chéo task. Lối tắt là khuếch đại tật chép ứng viên cuối (từ $0{,}71$ lên $0{,}81$) và học “tự giải lại” thay vì chọn.

\item[\textbf{Thứ năm.}] Credit-RL (thưởng bằng phần đóng góp Shapley biên) không giúp vai trò nào cải thiện theo chuẩn 5/5 fold. Planner sập về plan rỗng trong 200/200 câu; verifier có 23 lần sửa và 24 lần phá.

\item[\textbf{Thứ sáu.}] Credit-RL giai đoạn 2 (thiết kế chống lối tắt) cho V-COND $+3{,}0$ điểm (4/5 fold dương) trên GSM8K, là biến thể dương duy nhất, nhưng không chuyển miền: trên MATH cho $-7{,}0$ điểm (5/5 fold âm).

\item[\textbf{Thứ bảy.}] MAPoRL đồng huấn luyện S/V/A cho kết quả không đổi ($0{,}690 \to 0{,}690$), vì aggregator bị nghẽn suốt quá trình huấn luyện.
\end{description}

Mỗi phương pháp đều có thiết kế chốt trước hoặc cổng hiệu lực; chi tiết và đính chính
từng lần chạy nằm trong \texttt{docs/}. Phương pháp thứ tám --- bộ phân loại supervised
--- đã trình bày ở Bảng~\ref{tab:clf}.

\looseness=-1
Hình~\ref{fig:grpoexample} minh hoạ một ca cụ thể. Đáng chú ý nhất là thí nghiệm bất đối xứng thông tin (phương pháp thứ ba), khi đo được đồng thời hai mốc trên cùng một lần chạy. So với model yếu, hiệu ứng là $+17{,}0$ điểm (5/5 fold dương); so với chính model kiểm khi không xem bài, hiệu ứng là $-10{,}4$ điểm (5/5 fold âm). Đây là bản tái lập bằng huấn luyện của hiện tượng đổi dấu theo mốc so sánh (\S\ref{sec:synthesis}): RL thu hẹp thiệt hại tiếp xúc (gỡ 38\%) nhưng không đảo được dấu của $\dhonest$. Kết luận chung của toàn bộ các phương pháp nối thẳng vào \S\ref{sec:kappa}: khi tín hiệu thưởng cũng chỉ là một tín hiệu học được, quá trình tối ưu hoá tìm đến lối tắt của tín hiệu nhanh hơn tìm đến năng lực kiểm tra thực sự.

\section{Tổng hợp: hiệu ứng đo trên hai mốc so sánh}\label{sec:synthesis}

Hai quan sát ở trên thoạt nhìn ngược nhau: \S\ref{sec:roles} ghi nhận chênh lệch năng lực lớn
đi kèm mức tăng $+14{,}0$ điểm, còn \S\ref{sec:termG} lại chỉ ra chênh lệch lớn khiến mức cải
thiện tối đa $\dceil$ chuyển sang âm.

Chúng không mâu thuẫn --- chỉ là hai phép đo trên hai mốc khác nhau, và chênh lệch năng lực tác
động lên hai mốc theo hai chiều ngược nhau (Hình~\ref{fig:twobaselines}):
...
\item \emph{So sánh với model mạnh chạy độc lập $I$} (mốc quy chiếu thực tế trong triển khai):
      ...
      \S\ref{sec:baseline}).
\end{itemize}
Như vậy, hiện tượng trên không xuất phát từ cơ chế phối hợp mà do sự thiên lệch trong cách chọn
mốc so sánh: chênh lệch năng lực làm phóng đại hiệu quả biểu kiến so với model yếu, nhưng lại
làm suy giảm giá trị gia tăng thực tế so với model mạnh. Cơ chế tổn thất do tiếp xúc
...
\end{tikzpicture}
\caption{Hai mốc so sánh, hai chiều biến thiên. Trục hoành là khoảng cách năng lực: hiệu ứng đo so với
...
nhanh và đổi dấu sang âm khi vượt ngưỡng tới hạn $g^\ast \approx 0{,}09$ do tổn thất tiếp xúc $L$
...
(3) \emph{về cơ chế}, tổn thất bắt nguồn trực tiếp từ việc tiếp xúc với artifact sai lệch,
...
loại ước tính; và (3) tổn thất tiếp xúc $L$, phát sinh từ việc tiếp xúc với artifact sai lệch,
...
\item \textbf{Độ nhạy thống kê của một số nhóm đối chứng}: Thử nghiệm hoán vị vai trò trên MATH có
      ...
      điều kiện tiếp xúc, và so sánh giữa các model khác nhau trong cùng một họ kiến trúc vẫn
...
Toàn bộ mã nguồn, dữ liệu thô và nhật ký thí nghiệm được công bố tại\\
