...
lượt giải & một lần model sinh trọn một lời giải --- đơn vị đếm ngân sách suy luận\\
...
\texttt{maj@}$k$; \texttt{oracle@}$k$ & bỏ phiếu đa số trong $k$ lượt giải; trần lý tưởng
...
\paragraph{Bộ tổng hợp và các mốc.} Một \emph{lượt giải} là một lần model sinh trọn vẹn một
lời giải cho bài toán; đây cũng là đơn vị đếm ngân sách suy luận của báo cáo. Với $k$ lượt giải
độc lập cho cùng một bài:
...
\bottomrule
\end{tabular}
\caption{So sánh pipeline đầy đủ bốn vai trò với một lượt giải duy nhất (Qwen-1.5B; đo một lần trên
...
Mục tiếp theo áp dụng phép kiểm soát thứ nhất: lợi ích này đến từ sự phối hợp giữa các vai trò hay chỉ đơn thuần là do số lượng lượt giải tăng lên?

% ---------------------------------------------------------------
\subsection{Thêm lượt giải tạo giá trị, thêm vai trò tổng hợp thì không}\label{sec:budget}

\begin{table}[h]\centering\small
\begin{tabular}{@{}l c c@{}}
\toprule
Bộ tổng hợp (cùng 8 lượt giải) & MATH 1.5B & MATH 7B\\
\midrule
...
\bottomrule
\end{tabular}
\caption{So sánh các bộ tổng hợp với cùng ngân sách 8 lượt giải (hàng trên cùng là mốc 1 lượt để tham chiếu). Các ô là độ chính xác, thang 0--1.}
\label{tab:budget}
\end{table}
...
\emph{Thứ ba}, một đối chứng tách được ảnh hưởng của ``phối hợp'' khỏi ảnh hưởng của ``thêm lượt giải''. Khi cho giải lại mà \emph{không} xem phê bình của verifier, kết quả bằng đúng giải lại \emph{có} xem phê bình ($\texttt{rerun} = \texttt{loop}$, cùng cho $0{,}453$). Lợi ích vì vậy đến từ lượt giải thêm, không từ nội dung phản hồi. (Một phép đo đơn lẻ trước đó cho $+20$ điểm nhưng không tái lập: trên 5 fold chỉ còn $+4{,}0$ điểm.) Nhất quán với điều đó, biến thể vòng lặp giải-rồi-chấm --- rẻ hơn pipeline đủ vai trò --- còn thắng nó trên MATH: $+4{,}0$ điểm (KTC $[+0{,}5;+7{,}5]$; $t=3{,}21$) ở cùng chi phí.
...
Sau phép kiểm soát chi phí, yếu tố chắc chắn có giá trị duy nhất là ``các lượt giải độc lập bổ sung''. Mục tiếp theo đặt câu hỏi: những lượt giải đó nên do vai trò nào và model cỡ nào đảm nhiệm?
...
\bottomrule
\end{tabular}
\caption{Phân tầng theo số lượt đúng trong 5 lượt giải (MATH 1.5B, $n=150$). Cột $\Delta$ là chênh lệch độ chính xác giữa \texttt{maj@5} và Solver.}
\label{tab:strata}
\end{table}
...
\emph{Hết mạch thứ nhất.} Bức tranh sau ba phép kiểm soát: giá trị dương chắc chắn duy nhất là lượt giải thêm; cấu hình sửa-bài-của-nhau thua model mạnh đơn lẻ. Hồi II xác định giá trị mất đi đâu và có quy luật nào đoán trước được hay không.
...
Trên HumanEval, cùng model, cùng 4 lượt giải, chỉ khác nhau ở nguồn kiểm tra, kết quả được trình bày trong Bảng~\ref{tab:exec}.
