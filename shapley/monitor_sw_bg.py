#!/usr/bin/env python3
# Monitor 2 kernel interleaved. Poll 5', exit khi cả 2 final.
import json, os, subprocess, time, sys
ROOT=os.path.dirname(os.path.abspath(__file__)); os.chdir(ROOT)
JOBS=["sw_g15","sw_g7","sw_m15","sw_m7"]; LOG=open("monitor_sw.log","a")
...
for it in range(48):
    ...
    say(f"[{it}|{nf}/4] "+" ".join(f"{j}:{res[j][0]}" for j in JOBS))
    if nf==4:
        json.dump({j:{"status":res[j][0],"summary":res[j][1]} for j in JOBS},open("monitor_sw_result.json","w"),indent=2)
        say("ALL 4 DONE"); sys.exit(0)
    time.sleep(300)
json.dump({j:{"status":res[j][0],"summary":res[j][1]} for j in JOBS},open("monitor_sw_result.json","w"),indent=2)
