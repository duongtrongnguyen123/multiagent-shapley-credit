...
SMALL_DIR = os.path.dirname(find_one("/kaggle/input/**/model.safetensors", "1.5B"))
BIG_DIR   = os.path.dirname(find_one("/kaggle/input/**/model.safetensors.index.json", "7B"))
...
def use(mdir):
    if _cur["dir"] != mdir:
        if _cur["model"] is not None:
            del _cur["model"]; _cur["model"] = None; torch.cuda.empty_cache()
        tk = AutoTokenizer.from_pretrained(mdir); tk.padding_side = "left"
        if tk.pad_token is None: tk.pad_token = tk.eos_token
        t0 = time.time()
        m = AutoModelForCausalLM.from_pretrained(mdir, torch_dtype=torch.float16, device_map="auto").eval()
        _cur.update(dir=mdir, model=m, tok=tk)
        print(f"  loaded {os.path.basename(mdir)} in {time.time()-t0:.0f}s", flush=True)
...
def gen(prompts, max_new, mdir, batch):
    ...
    for i in range(0, len(prompts), batch):
        ...
        enc = tk(chunk, return_tensors="pt", padding=True).to(model.device)
        with torch.no_grad():
            o = model.generate(**enc, max_new_tokens=max_new, do_sample=False,
                               pad_token_id=tk.pad_token_id)
        outs += tk.batch_decode(o[:, enc["input_ids"].shape[1]:], skip_special_tokens=True)
    return outs
...
if P:
    plans = gen([chat(md("P"), PLAN_SYS, q) for q in questions], 512, md("P"), bs("P"))
if S:
    sol = gen([chat(md("S"), SOLVE_SYS, solve_user(i)) for i in range(n)], 1024, md("S"), bs("S"))
...
if V:
    ...
    ver = gen(prompts, 1024, md("V"), bs("V"))
...
if A:
    ...
    agg = gen(prompts, 1024, md("A"), bs("A"))
...
summary = {"config_id": CONFIG_ID, "mask": {"P": P, "S": S, "V": V, "A": A},
           "dataset": "MATH-500", "big_role": BIG_ROLE, "n": n, "correct": correct, "accuracy": acc,
