...
PRE_ACC=pos/len(DATA)
# ---- H28d (#37): do RO RI dung cach — CUNG 60 bai, truoc va sau huan luyen ----
PROBE_Q=[r["question"] for r in TEROWS[:60]]; PROBE_G=[gold(r) for r in TEROWS[:60]]
_pp=gen(S_SYS,PROBE_Q,400,0.0)
PROBE_PRE=sum(ok(pred(x),g) for x,g in zip(_pp,PROBE_G))/60
print(f"PROBE_PRE={PROBE_PRE:.4f}",flush=True)
...
_qq=gen(S_SYS,PROBE_Q,400,0.0)
PROBE_POST=sum(ok(pred(x),g) for x,g in zip(_qq,PROBE_G))/60
LEAK=round(PROBE_PRE-PROBE_POST,4)
POST_ACC=PROBE_POST
print(f"PROBE_POST={PROBE_POST:.4f} LEAK={LEAK} (CUNG 60 bai)",flush=True)
...
out={"tag":"disc","auc":auc,"VALID_auc":bool(auc and auc>0.55),
     "probe_pre":round(PROBE_PRE,4),"probe_post":round(PROBE_POST,4),"adapter_leak":LEAK,"probe_n":60,
