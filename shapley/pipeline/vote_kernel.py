...
out["arms"]["PSVA"]={"acc":acc(fa)}
print(f"[PSVA] {json.dumps(out['arms']['PSVA'])}",flush=True)

# Mẫu Solver thêm (nhiệt độ) để có phiếu
def sample(k):
    outs=[[] for _ in qs]
    for i in range(0,len(qs),BS):
        ch=qs[i:i+BS]
        ps=[tok.apply_chat_template([{"role":"system","content":SOLVE},{"role":"user","content":f"{q}\n\nPlan:\n{p}"}],
             tokenize=False,add_generation_prompt=True) for q,p in zip(ch,plans[i:i+BS])]
        e=tok(ps,return_tensors="pt",padding=True).to(model.device)
        with torch.no_grad():
            o=model.generate(**e,max_new_tokens=768,do_sample=True,temperature=0.8,top_p=0.95,
                             num_return_sequences=k,pad_token_id=tok.pad_token_id)
        L=e["input_ids"].shape[1]
        for j in range(len(ch)):
            for r in range(k): outs[i+j].append(pred(tok.decode(o[j*k+r,L:],skip_special_tokens=True)))
        print(f"    sample {min(i+BS,len(qs))}/{len(qs)}",flush=True)
    return outs
extra=sample(3)

from collections import Counter
def vote(cands):
    ks=[norm(c) for c in cands if norm(c)]
    if not ks: return None
    k,_=Counter(ks).most_common(1)[0]
    for c in cands:
        if norm(c)==k: return c
    return None
# 3 phiếu: Solver greedy + Verifier + 1 mẫu
v3=[vote([sa[i], va[i], extra[i][0]]) for i in range(len(qs))]
out["arms"]["PSV_vote3"]={"acc":acc(v3),
    "differs_from_V":sum(1 for i in range(len(qs)) if norm(v3[i])!=norm(va[i]))}
print(f"[PSV_vote3] {json.dumps(out['arms']['PSV_vote3'])}",flush=True)
# 5 phiếu: Solver greedy + Verifier + 3 mẫu
v5=[vote([sa[i], va[i]]+extra[i][:3]) for i in range(len(qs))]
out["arms"]["PSV_vote5"]={"acc":acc(v5),
    "differs_from_V":sum(1 for i in range(len(qs)) if norm(v5[i])!=norm(va[i]))}
print(f"[PSV_vote5] {json.dumps(out['arms']['PSV_vote5'])}",flush=True)

out["V_gain"]=round(out["arms"]["PSV"]["acc"]-out["acc_PS"],4)
out["llmagg_vs_PSV"]=round(out["arms"]["PSVA"]["acc"]-out["arms"]["PSV"]["acc"],4)
out["vote3_vs_PSV"]=round(out["arms"]["PSV_vote3"]["acc"]-out["arms"]["PSV"]["acc"],4)
out["vote5_vs_PSV"]=round(out["arms"]["PSV_vote5"]["acc"]-out["arms"]["PSV"]["acc"],4)
out["vote5_vs_llmagg"]=round(out["arms"]["PSV_vote5"]["acc"]-out["arms"]["PSVA"]["acc"],4)
