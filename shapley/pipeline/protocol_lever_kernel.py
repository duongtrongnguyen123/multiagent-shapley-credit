# H98 (dang ky truoc #108) — B CO PHAI DON BAY GIAO THUC?
# 2 cap (P: dscoder->qwen7b, Q: qwen1.5b->qwen7b) x 3 giao thuc sua (R0/R1/R2), CUNG MBPP 11-510.
...
t0 = time.time()
for tag in ["qwen1.5b", "dscoder", "qwen7b"]:      # nen: re -> dat
    print(f"\n=== NEN {tag} ===", flush=True)
...
run_gates = {"n>=480": N >= 480,
             "extract_min>=.80": min(ext.values()) >= .80,
...
for pk, (s, i) in PAIRS.items():
    ...
    pair_gates[pk] = {"I-S>=.02": acc[i]-acc[s] >= .02, "p(I-S)<.05": p_is < .05,
                      "A>=.02 (chi cap P)": (A >= .02) if pk == "P" else True,
...
# --- DAI LUONG CHINH: D = acc(CEIL_R1|R2) - acc(CEIL_R0), McNemar GHEP CAP ---
D = {}
for pk in PAIRS:
...
if VOID:
    ...
    print("  -> khong co o nao so sanh duoc")
else:
    valid = {k: v for k, v in D.items() if pair_ok[k.split(":")[0]]}
    pos = {k: v for k, v in valid.items() if v["D"] >= .02 and v["p"] < .05}
    neg = {k: v for k, v in valid.items() if v["D"] <= -.02 and v["p"] < .05}
    pk_pos = {k.split(":")[0] for k in pos}; pk_neg = {k.split(":")[0] for k in neg}
    # hang 4 = duong o CAP NAY va am o CAP KIA (khong phai hai giao thuc trong cung mot cap)
    if pos and neg and (pk_pos - pk_neg) and (pk_neg - pk_pos):
        print(f"  -> HANG 4: DOI DAU theo cap. duong={sorted(pos)} am={sorted(neg)}")
        print("     => khong co loi nhac tot pho quat; dieu tra theo r*.")
    elif pos:
        print(f"  -> HANG 1: B LA DON BAY GIAO THUC. {sorted(pos)}")
        for k in pos: print(f"       {k}: D={pos[k]['D']:+.4f} p={pos[k]['p']:.4g} "
                            f"(dB={pos[k]['dB']:+.4f}, dC={pos[k]['dC']:+.4f})")
        print("     => PHAI rut phan '#181: giao thuc CHON thang vi dat B=0 theo cau truc'.")
    elif neg:
        print(f"  -> HANG 3: B DICH CHUYEN DUOC nhung chi theo chieu XAU. {sorted(neg)}")
        print("     => van bac 'B la hang cua cap model'. Canh bao trien khai.")
    else:
        print("  -> HANG 2: B KHONG dich chuyen duoc bang loi nhac (voi BA giao thuc nay).")
        print("     => manh them cho doc 'sua chet bang so hoc'. KHONG duoc suy ra moi giao thuc.")
