# H85 (dang ky truoc #94) — chay lai H63: k=8->4, LUU RAW that sau moi nhanh — REFACTOR bang CHON LOC, khong phai SUA CHUA.
# ref1 | ref_exec3 (tai lap H53) | ref_sel4 (loc bang test, xep hang bang nut AST) | ref_sel4_first
...
json.dump({"partial": True, "done": ["ref1"], "raw": {"ref1": r1}}, open(f"/kaggle/working/partial_{RUN}.json","w"))
...
json.dump({"partial": True, "done": ["ref1","exec3"], "raw": {"ref1": r1, "exec3": e3}}, open(f"/kaggle/working/partial_{RUN}.json","w"))
...
print(f"{K} mau + chay test xong ({time.time()-t0:.0f}s)", flush=True)
json.dump({"partial": True, "done": ["ref1","exec3","cands"], "raw": {"ref1": r1, "exec3": e3, "CAND": {str(i): CAND[i] for i in KEEP}}}, open(f"/kaggle/working/partial_{RUN}.json","w"))
...
rows = [score("ref1", r1, p1), score("ref_exec3", e3, pe3),
        score("ref_sel4", {i: (sel[i] or "") for i in KEEP}, okmap_sel),
        score("ref_sel4_first", {i: (selfirst[i] or "") for i in KEEP}, okmap_sel)]
...
d = G["ref_sel4"] - G["ref_exec3"]
dfirst = G["ref_sel4"] - G["ref_sel4_first"]
print(f"\n  so mau DAT / bai: TB {res['mean_npass']}/{K} | {res['zero_pass']} bai KHONG mau nao dat")
print(f"  good(sel4) - good(exec3)      = {d:+.4f}   <-- DAI LUONG CHINH (bang khoa #68)")
print(f"  good(sel4) - good(sel4_first) = {dfirst:+.4f}   <-- LOC hay XEP HANG?")
...
if P["ref_sel4"] < .90: print(f"  -> them: preserve(sel4) = {P['ref_sel4']:.4f} < .90 -> GIOI HAN NANG LUC, khong phai gioi han tim kiem.")
