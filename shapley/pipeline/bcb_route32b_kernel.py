# H79 (dang ky truoc #88) — DINH TUYEN HOP LY tren BigCodeBench, cheap=7B, strong=32B, bf16.
...
M = {"7B": find_model("2-5-7b", "qwen2-5-7b"), "32B": find_model("32b")}
BCB = sorted(glob.glob("/kaggle/input/**/bigcodebench_v014.json", recursive=True), key=len)[0]
...
if VRAM < 80: raise SystemExit(f"HUY: can >=80 GB, chi co {VRAM:.1f}")

RAWALL = json.load(open(BCB))[:N_TASK]
def canon(r): return r["complete_prompt"] + r["canonical_solution"]   # body CAN complete_prompt (da kiem: 0/40 vs 32/40)
...
def official_raw(rec, code):
    ...
    return (re.search(r"^OK", out, re.M) is not None) or ("FAILED" not in out and rc == 0)
