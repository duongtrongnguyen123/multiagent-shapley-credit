# H93 (dang ky truoc #103) — HO hay chi la TRONG SO KHAC?
# Ba pool, MOT lan chay, bo chon GIU NGUYEN (test do Qwen-7B viet mot lan).
# A = 3 mau Qwen-7B (cung trong so) | B = Qwen+Llama+DeepSeek (khac HO) | C = Qwen 1.5B/7B/14B (khac CO)
...
def _h(t): return _hl.md5(_nrmcode(t).encode()).hexdigest()

def diversity(pool):
    """#99: do da dang VE CHUOI — khong cham diem, chuan hoa da khoa o #136/#99."""
    hs = [[_h(CODE[k][i]) for k in pool] for i in range(N)]
    return {"distinct": round(sum(len(set(h)) for h in hs)/N, 4),
            "solo": round(sum(1 for h in hs if len(set(h)) == 1)/N, 4)}
...
def mcnemar(a, b):
    ...
    n_ = b01 + b10
    if n_ == 0: return b01, b10, 1.0
    k_ = min(b01, b10)
    return b01, b10, round(min(1.0, 2.0*sum(_m.comb(n_, i) for i in range(k_+1))/(2.0**n_)), 6)
...
POOLS = {"A_sampling": ["Q1", "Q2", "Q3"], "B_family": ["Q1", "L", "D"],
         "C_size": ["Q1", "XS", "XL"]}   # #103: cung ho, khac CO
...
res = {"tag": RUN, "n": N, "pools": RES, "test_copy_rate": copy_rate,
       ...
       "mcnemar": {
         "H_C_vs_A": dict(zip(("b01","b10","p"), mcnemar(
             ...
             [any(PASS[k][i] for k in POOLS["C_size"]) for i in range(N)]))),
         "H_B_vs_C": dict(zip(("b01","b10","p"), mcnemar(
             [any(PASS[k][i] for k in POOLS["C_size"]) for i in range(N)],
             [any(PASS[k][i] for k in POOLS["B_family"]) for i in range(N)]))),
       }}
