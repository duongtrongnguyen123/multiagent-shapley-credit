...
_tr=sorted(glob.glob("/kaggle/input/**/math_500_test.csv",recursive=True),key=len)
_te=sorted(glob.glob("/kaggle/input/**/math_500_test.csv",recursive=True),key=len)
_all=list(csv.DictReader(open(_tr[0])))
TRROWS=_all[:NTR]
TEROWS=_all[NTR:NTR+NTE]   # TACH DOI, khong ro ri
...
def _bx(t):
    ...
    return None
gold=lambda r:(_bx(r["Answer"]) or (NUM.findall(r["Answer"]) or [""])[-1])
...
@torch.no_grad()
def gen(sysm,usrs,mx=400,temp=0.0,k=1):
    outs=[]
...
for k in ["greedy1","maj8","rerank8","wvote_sum","wvote_mean","oracle8",
          "rerank_minus_maj","wsum_minus_maj","wmean_minus_maj","wsum_pct_gap","pct_gap_closed"]: out[k]=sp(k)
