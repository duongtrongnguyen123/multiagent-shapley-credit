# H44 (dang ky truoc #50) — mo neo hay cau truc tuan tu? A/B/C tren CUNG nhom escalate.
...
if ESC:
    ...
    # ---- NHANH B (#50): tuan tu KHONG MO NEO. Cung cau truc, cung chi phi, chi bo code cu ----
    fresh = {i: PR[i] for i in ESC}
    n1 = parallel_gen(bigs, T7, SOLVE, fresh, BSB, 0.0, 1)
    rev2 = {i: f"{PR[i]}\n\nCode:\n```python\n{extract(n1[i][0])}\n```" for i in ESC}
    n2 = parallel_gen(bigs, T7, REVIEW, rev2, BSB, 0.0, 1)
    for i in ESC:
        NOANCH_CODE[i] = extract(n2[i][0]) or extract(n1[i][0])
    rs2 = probe_many([(ALL[i], NOANCH_CODE[i]) for i in ESC])
    NOANCH_PR = {i: rs2[n] for n, i in enumerate(ESC)}
...
for i in MINE:
    ...
    out["items"].append({
        ...
        "noanchor": ({k: NOANCH_PR[i][k] for k in ("out", "a0", "held", "compiles")} if i in NOANCH_PR else None),
