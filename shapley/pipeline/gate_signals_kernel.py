# H95 (dang ky truoc #105) — TIN HIEU CONG NAO lay duoc du dia KHAC HO?
# #169: cap khac ho co Delta_ceil=+.0421 (CO du dia) nhung cong kha thi lay ~0.
# Thu BA cong tren CUNG artifact: z_self (test cua S) | z_dear (test cua M) | z_agree (dong thuan)
# (goc: H88) — SUA CO CONG: D co bien mat khi cong bang tin hieu doc lap khong?
...
# #105: test do model DAT viet — tin hieu cong THU HAI, doc lap ve HO voi artifact cua S
...
Z  = par(own_tests, [(S[i], TESTS[i]) for i in range(N)])          # z_self
Z_DEAR = par(own_tests, [(S[i], TESTS_DEAR[i]) for i in range(N)])  # z_dear (#105)
_ni = [extract(t) for t in I_raw]
Z_AGREE = [bool(S[i].strip()) and S[i].strip() == _ni[i].strip() for i in range(N)]  # z_agree (#105)
...
# #105: ba cong KHA THI, cung artifact, chi doi TIN HIEU
GATES = {"z_self": Z, "z_dear": Z_DEAR, "z_agree": Z_AGREE}
gate_res = {}
for _gn, _gv in GATES.items():
    _g = [PS[i] if _gv[i] else PV[i] for i in range(N)]
    _b01, _b10, _p = mcnemar(PI, _g)
    gate_res[_gn] = {"acc": A(_g), "delta_honest": round(A(_g) - A(PI), 4),
...
print("\n-- BANG KHOA #105 --")
print(f"  do phu: z_self={cov_self} z_dear={cov_dear}")
for _gn, _gr in gate_res.items():
    print(f"  {_gn:8s} acc={_gr['acc']:.4f} d_honest={_gr['delta_honest']:+.4f} p={_gr['mcnemar']['p']:.4g} (dat cong {_gr['pass_rate']:.3f})")
_best = max(gate_res.items(), key=lambda kv: kv[1]["delta_honest"])
_win = [g for g, r in gate_res.items() if r["delta_honest"] >= .02 and r["mcnemar"]["p"] < .05]
_harm = [g for g, r in gate_res.items() if r["delta_honest"] <= -.02 and r["mcnemar"]["p"] < .05]
if VOID: print(f"  -> HANG 0: VOID {VOID}")
elif not (d_ceil >= .02 and mc["ceil"][2] < .05):
    print(f"  -> HANG 3: KHONG tai lap duoc du dia cua #169 (d_ceil={d_ceil:+.4f} p={mc['ceil'][2]:.3g}) => phai HA CAP #169")
elif _win: print(f"  -> HANG 1: kappa SUA DUOC — cong {_win} lay duoc du dia")
else: print(f"  -> HANG 2: kappa LA NUT THAT THAT — ca ba tin hieu deu khong lay duoc (tot nhat: {_best[0]} {_best[1]['delta_honest']:+.4f})")
if _harm: print(f"  -> HANG 4 (them): cong {_harm} CHU DONG HAI")
