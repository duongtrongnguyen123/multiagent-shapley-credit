...
mo, tk = load("dear")
I_raw = gen(mo, tk, SOLVE, Q, BSZ["dear"])
print(f"I xong ({time.time()-t0:.0f}s)", flush=True)
save_partial(S_raw=S_raw, S2_raw=S2_raw, I_raw=I_raw)
VP = [f"{Q[i]}\n\nProposed solution:\n{S_raw[i]}" for i in range(N)]
V_raw = gen(mo, tk, REVIEW, VP, BSZ["dear"])
print(f"V xong ({time.time()-t0:.0f}s)", flush=True)
mo = None; tk = None
free()
save_partial(S_raw=S_raw, S2_raw=S2_raw, I_raw=I_raw, V_raw=V_raw)
...
print("\n-- BANG KHOA #97 (CHI hang 0 va 1, theo sua doi #97-b) --")
if VOID:
