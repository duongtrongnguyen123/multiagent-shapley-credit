...
for fi in range(NF):
    ...
    va=[pred(x) for x in vers]
    accV=acc(va)
    up=[f"{q}\n\nCandidate 1:\n{s}\n\nCandidate 2:\n{v}" for q,s,v in zip(qs,sols,vers)]
    # A_base
    ab_txt=gen(AGG,up,512); ab=[pred(x) for x in ab_txt]
    # A_fallback: khong co \boxed -> lay dap an Verifier (KHONG goi lai model)
    af=[(va[i] if "\\boxed" not in (ab_txt[i] or "") else ab[i]) for i in range(len(qs))]
    # A_forced: ep dinh dang, 64 token
    AGGF="Reply with ONLY the final answer in \\boxed{}. No explanation, no discussion."
    ax_txt=gen(AGGF,up,64); ax=[pred(x) for x in ax_txt]
    # A_both
    abo=[(va[i] if "\\boxed" not in (ax_txt[i] or "") else ax[i]) for i in range(len(qs))]
    bx=lambda L: round(sum(1 for x in L if "\\boxed" in (x or ""))/len(L),3)
    r={"fold":fi,"n":len(qs),"V":accV,
       "A_base":acc(ab),"A_fallback":acc(af),"A_forced":acc(ax),"A_both":acc(abo),
       "gain_base":round(acc(ab)-accV,4),"gain_fallback":round(acc(af)-accV,4),
       "gain_forced":round(acc(ax)-accV,4),"gain_both":round(acc(abo)-accV,4),
       "boxed_base":bx(ab_txt),"boxed_forced":bx(ax_txt)}
...
out={"task":TASK,"n_folds":NF,"fold_n":FOLD,"folds":folds,
     "V":spread("V"),"A_base":spread("A_base"),"A_fallback":spread("A_fallback"),
     "A_forced":spread("A_forced"),"A_both":spread("A_both"),
     "gain_base":spread("gain_base"),"gain_fallback":spread("gain_fallback"),
     "gain_forced":spread("gain_forced"),"gain_both":spread("gain_both"),
     "boxed_base":spread("boxed_base"),"boxed_forced":spread("boxed_forced")}
