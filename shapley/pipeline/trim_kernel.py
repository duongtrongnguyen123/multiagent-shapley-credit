...
CHARS={"FULL":0,"MIN":0,"TRIM":0}      # đếm chi phí context
...
# ---------- TRIM: GIU Planner->Solver, chi cat trace o V va A ----------
sT=sF                                        # DUNG LAI dung loi giai cua FULL -> chi doi 1 bien
...
# ---------- NOVA: bo han Verifier va Aggregator, chi P->S ----------
accNV=acc(saT)
print(f"[NOVA] acc={accNV}",flush=True)

out={"task":TASK,"n":len(gs),
     "acc_solver_only":acc(solo),
     "acc_full":accF,"acc_trim":accT,"acc_nova":accNV,
     "chars_full":CHARS["FULL"],"chars_trim":CHARS["TRIM"],
     "trim_minus_full":round(accT-accF,4),
     "nova_minus_full":round(accNV-accF,4),
