...
AGG  =f"You are given candidate solutions. Decide the single correct final answer. {TAIL}"
...
for fi in range(NF):
    ...
    # --- 4 luot: maj@4 (doi chung cung ngan sach) ---
    S4=S+[gen(SOLVE,Q,512,0.8,"maj4")]      # tai su dung 3 mau cu + 1 mau moi
    a_mj4=[]
    for i in range(len(Q)):
        c={}
        for r_ in range(4):
            p=pred(S4[r_][i])
            if p is not None: c[p]=c.get(p,0)+1
        a_mj4.append(max(c,key=c.get) if c else None)
    # --- 4 luot: P -> 3S (ba solver CUNG doc mot ke hoach) -> dem phieu ---
    pl2=gen(PLAN,Q,384,0.0,"P3S")
    P3=[gen(SOLVE,[f"{q}\n\nPlan:\n{p}" for q,p in zip(Q,pl2)],512,0.8,"P3S") for _ in range(3)]
    a_p3s=[]
    for i in range(len(Q)):
        c={}
        for r_ in range(3):
            p=pred(P3[r_][i])
            if p is not None: c[p]=c.get(p,0)+1
        a_p3s.append(max(c,key=c.get) if c else None)
    # --- 4 luot: P -> S -> V -> A (dung lai pl/so/ve cua PSV, chi them Aggregator) ---
    agg=gen(AGG,[f"{q}\n\nCandidate A:\n{s_}\n\nCandidate B:\n{v_}"
                 for q,s_,v_ in zip(Q,so,ve)],512,0.0,"PSVA")
    a_psva=[pred(x) if pred(x) is not None else pred(v_) for x,v_ in zip(agg,ve)]
    r={"fold":fi,"n":len(Q),"greedy1":acc(g1),"maj3":acc(a_mj),"PSV":acc(a_psv),
       "maj4":acc(a_mj4),"P3S":acc(a_p3s),"PSVA":acc(a_psva),
       "P3S_minus_maj4":round(acc(a_p3s)-acc(a_mj4),4),
       "PSVA_minus_PSV":round(acc(a_psva)-acc(a_psv),4),
       "maj4_minus_maj3":round(acc(a_mj4)-acc(a_mj),4),
...
for k in ["greedy1","maj3","PSV","SVV","SS_anc","maj4","P3S","PSVA",
          "maj3_minus_PSV","SVV_minus_maj3","SSanc_minus_PSV",
          "P3S_minus_maj4","PSVA_minus_PSV","maj4_minus_maj3"]:
    out[k]=sp(k)
