# H46 (dang ky truoc #52) — them nhanh B: tuan tu KHONG MO NEO, cung lan chay.
# (dan xuat tu H45 / #51) — delta_seq = seq - maj3, do tren luoi tac vu x co model.
...
# ---- NHANH B (#52): tuan tu KHONG MO NEO. Cung cau truc, cung 3 luot, chi bo dap an cu ----
b2 = pgen(MS, SOLVE, Q, 0.0, 1)                      # giai lai MOI, khong nhac dap an truoc
vb_ = {i: f"{Q[i]}\n\nProposed solution:\n{b2[i][0]}" for i in MINE}
b3 = pgen(MS, VERIFY, vb_, 0.0, 1)
print("xong sinh (A + B)", flush=True)

CAP = 3000
out = {"tag": f"H46s{SHARD}", "shard": SHARD, "nshard": NSHARD, "task": TASK, "size": SIZE,
...
for i in MINE:
    out["items"].append({
        ...
        "seq_noanchor_pred": (pred(b3[i][0]) if pred(b3[i][0]) is not None else pred(b2[i][0])),
        "seq_texts": {"s1": s1[i][0][:CAP], "s2": s2[i][0][:CAP], "s3": s3[i][0][:CAP]},
    })
json.dump(out, open(f"/kaggle/working/res_H46s{SHARD}.json", "w"))
print(f"DA LUU {len(out['items'])} bai -> res_H46s{SHARD}.json", flush=True)
