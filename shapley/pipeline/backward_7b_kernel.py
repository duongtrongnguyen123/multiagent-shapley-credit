...
_c = glob.glob("/kaggle/input/**/model.safetensors.index.json", recursive=True) or glob.glob("/kaggle/input/**/model.safetensors", recursive=True)
...
subprocess.run([_sys.executable, "-m", "pip", "install", "-q", "-U", "bitsandbytes>=0.46.1"], check=False)
...
models = {}
for gpu in (0, 1):
    models[gpu] = AutoModelForCausalLM.from_pretrained(MODEL, quantization_config=_q,
                                                       device_map={"": gpu}).eval()
...
def run_pipe(gpu, plan_sys, qs, gs):
    """Chạy pipeline (psva | solvejudge) với plan từ plan_sys. Trả final solutions + ok."""
    n = len(qs)
    plans = run_planner(gpu, plan_sys, qs, PLAN_MX)
    wp = [f"{qs[i]}\n\nSuggested plan:\n{plans[i]}" for i in range(n)]
    if PIPE == "psva":
        sol = gen(gpu, SOLVE_SYS, wp, SOLVE_MX, temp=1.0)
        ...
        cur_s = gen(gpu, SOLVE_SYS, wp, SOLVE_MX, temp=1.0)   # S1 greedy with plan
        stop_at = [None] * n
        sols, judges = [], []
        for v in range(MAXV):
...
def process_fold(f):
    ...
    threads = []
    lock = threading.Lock()
    def worker(gpu, plan_sys, tag):
        final, plans, stop_at, calls, sols, judges = run_pipe(gpu, plan_sys, qs, gs)
        with lock:
            results[tag] = {"final": final, "plans": plans, "stop_at": stop_at,
                            "calls": calls, "sols": sols, "judges": judges}
    # forward GPU0, backward GPU1 song song
    th1 = threading.Thread(target=worker, args=(0, PLAN_FWD, "fwd"))
    th2 = threading.Thread(target=worker, args=(1, PLAN_BWD, "bwd"))
