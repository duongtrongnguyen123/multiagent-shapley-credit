# H53 (dang ky truoc #59) — oracle sua toi da BA VONG (H52 chi mot vong).
# (dan xuat tu H52 / #58) — REFACTOR: giu nguyen hanh vi co can ORACLE that khong?
...
BAD0 = [i for i in KEEP if not chk[i][0]]
print(f"ref1 lam HONG {len(BAD0)}/{len(KEEP)}", flush=True)
c_ex1 = dict(c1)          # oracle 1 vong (bang H52)
c_ex3 = dict(c1)          # oracle toi da 3 vong
rounds_used = {i: 0 for i in KEEP}
cur_err = {i: chk[i][1] for i in KEEP}
bad = list(BAD0)
for rd in range(3):
    if not bad: break
    fx = {i: "Task: " + task_text(ALL[i])[:600] + "\n\nCode:\n```python\n" + c_ex3[i] + "\n```\n\nError:\n" + cur_err[i][:600] for i in bad}
    Rf = pgen(MS, FIXERR, fx, 0.0, 1)
    still = []
    for i in bad:
        cand = extract(Rf[i][0]) or c_ex3[i]
        c_ex3[i] = cand
        rounds_used[i] = rd + 1
        if rd == 0: c_ex1[i] = cand        # anh chup sau DUNG mot vong
        ok_i, err_i = run_tests_err(ALL[i], cand)
        if not ok_i:
            cur_err[i] = err_i; still.append(i)
    print(f"  vong {rd+1}: con hong {len(still)}/{len(BAD0)}", flush=True)
    bad = still
c_exec = c_ex3
...
PE1 = run_many([(ALL[i], c_ex1[i]) for i in KEEP])
PE = run_many([(ALL[i], c_ex3[i]) for i in KEEP])
...
for k_, i in enumerate(KEEP):
    out["items"].append({
        ...
        "ref_exec1": {"pass": PE1[k_][0], "nodes": nodes_of(c_ex1[i])},
        "ref_exec3": {"pass": PE[k_][0], "nodes": nodes_of(c_ex3[i])},
