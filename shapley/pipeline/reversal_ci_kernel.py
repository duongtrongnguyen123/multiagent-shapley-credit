...
for fi in range(NF):
    ...
    # FULL: V va A thay TOAN VAN
    vF=gen(VERIFY,[f"{q}\n\nProposed solution:\n{s}" for q,s in zip(qs,sols)],768)
    aF=gen(AGG,[f"{q}\n\nCandidate 1:\n{s}\n\nCandidate 2:\n{v}" for q,s,v in zip(qs,sols,vF)],512)
    accF=acc([pred(x) for x in aF])
    # TRIM: CUNG loi giai, nhung V va A chi thay DAP AN
    vT=gen(VERIFY,[f"{q}\n\nProposed solution:\n{fmt(a)}" for q,a in zip(qs,sa)],768)
    vaT=[pred(x) for x in vT]
    aT=gen(AGG,[f"{q}\n\nCandidate 1: {a1}\nCandidate 2: {a2}" for q,a1,a2 in zip(qs,sa,vaT)],512)
    accT=acc([pred(x) for x in aT])
    r={"fold":fi,"n":len(qs),"solo":acc(sa),"FULL":accF,"TRIM":accT,
...
out={"task":TASK,"n_folds":NF,"fold_n":FOLD,"folds":folds,
     "solo":spread("solo"),"FULL":spread("FULL"),"TRIM":spread("TRIM"),
