#!/usr/bin/env python3
"""Gop cac shard H42 (dinh tuyen tren code, MBPP) — dang ky truoc #48.
...
for nm, a, c, p in [("small_1 (1 ban 1.5B)", small_1, 1.0, None),
                    ...
                    ("route_oracle", route_orac, cost_o, pe_o)]:
    print(f"{nm:22s}{a:11.4f}{c:10.2f}" + (f"{p:8.3f}" if p is not None else f"{'-':>8s}"))

print(f"\n-- doi chieu voi bang khoa #48 --")
print(f"  route_oracle - route_consensus = {route_orac-route_cons:+.4f}")
print(f"  route_consensus - big_maj3     = {route_cons-big_maj3:+.4f}   (re hon {3*RATIO/cost_c:.2f}x)")
print(f"  route_oracle   - big_maj3      = {route_orac-big_maj3:+.4f}   (re hon {3*RATIO/cost_o:.2f}x)")
