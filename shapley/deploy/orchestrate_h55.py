...
TEMPLATE = (ROOT / "pipeline" / "mbpp_tdd_kernel.py").read_text()
NSHARD = int(os.environ.get("NSHARD", "20"))
ROUND = os.environ.get("ROUND", "h55")
...
def main():
    ...
    for s in range(NSHARD):
        ...
        src = (TEMPLATE.replace("@@SHARD@@", str(s)).replace("@@NSHARD@@", str(NSHARD))
                .replace("@@RUN@@", "H55").replace("@@TIDLO@@", "11").replace("@@TIDHI@@", "510"))
        slug = f"spec-driven-synthesis-shard-{s:02d}"
        d = KDIR / f"s{s:02d}"
        d.mkdir()
