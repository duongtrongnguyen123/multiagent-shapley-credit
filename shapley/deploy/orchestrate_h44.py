...
TEMPLATE = (ROOT / "pipeline" / "code_anchor_kernel.py").read_text()
NSHARD = int(os.environ.get("NSHARD", "20"))
ROUND = os.environ.get("ROUND", "h44")
...
def main():
    ...
    for s in range(NSHARD):
        ...
        src = (TEMPLATE.replace("@@SHARD@@", str(s)).replace("@@NSHARD@@", str(NSHARD))
                .replace("@@RUN@@", "H44").replace("@@TIDLO@@", "11").replace("@@TIDHI@@", "510"))
        slug = f"code-repair-ablation-shard-{s:02d}"
        d = KDIR / f"s{s:02d}"
        d.mkdir()
