...
TEMPLATE = (ROOT / "pipeline" / "mbpp_select3_kernel.py").read_text()
NSHARD = int(os.environ.get("NSHARD", "20"))
ROUND = os.environ.get("ROUND", "h58")
...
def main():
    ...
    for s in range(NSHARD):
        ...
        src = (TEMPLATE.replace("@@SHARD@@", str(s)).replace("@@NSHARD@@", str(NSHARD))
                .replace("@@RUN@@", "H58").replace("@@TIDLO@@", "11").replace("@@TIDHI@@", "510"))
        slug = f"multi-spec-selection-shard-{s:02d}"
        d = KDIR / f"s{s:02d}"
        d.mkdir()
