#!/usr/bin/env python3
"""Deploy eval: P(minlen-GRPO)+S vs S solo trên GSM8K test.

  ACCOUNT=tbmdemi N=200 python deploy/orchestrate_ps_minlen_eval.py
"""
...
SRC = ROOT / "pipeline" / "ps_minlen_eval_kernel.py"
N = os.environ.get("N", "200")
...
def main():
    ...
    (d / "kernel-metadata.json").write_text(json.dumps(
        {"id": ref, "title": slug, "code_file": "kernel.py", "language": "python",
         ...
         "dataset_sources": ["xatri007/qwen2-5-1-5b-instruct",
                             "thedevastator/grade-school-math-8k-q-a", ADAPTER_DS],
    ...
    print(f"[{slug}] N={N} adapter={ADAPTER_DS} -> {user} "
          f"{'OK' if ok else 'FAIL'} "
          f"{(r.stdout or r.stderr).strip().splitlines()[-1][:70]}", flush=True)
    (ROOT / "manifest_credit_rl.json").write_text(json.dumps(
        [{"user": user, "token": token, "slug": slug, "ref": ref, "pushed": ok}],
        indent=2))
