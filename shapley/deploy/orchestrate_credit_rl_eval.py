...
ROLE="PSVA"           -> kernel mount 3 adapter S,V,A (P base); so pipeline P->S->V->A
                         với S,V,A trained vs toàn bộ base.
"""
...
def main():
    user, token = pick()
    if ROLE not in (*ROLE_DATASETS, "FULL", "PSVA"):
        raise SystemExit(f"ROLE={ROLE} invalid (P|S|V|A|FULL|PSVA)")
