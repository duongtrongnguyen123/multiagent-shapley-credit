#!/usr/bin/env python3
"""Push kernel mock test 2 GPU song song lên Kaggle T4."""
import os, re, json, shutil, subprocess
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
ACCOUNTS = Path(os.environ.get("ACCOUNTS_FILE", ROOT / "accounts.txt")).expanduser()
SRC = ROOT / "pipeline" / "mock_2gpu_kernel.py"
ACCOUNT = os.environ.get("ACCOUNT", "TrgDinKai")
KDIR = ROOT / "kernels_mock2gpu"
...
def main():
    ...
    print(f"[{slug}] -> {user} {'OK' if ok else 'FAIL'} "
