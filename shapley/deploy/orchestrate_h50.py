...
TEMPLATE = (ROOT / "pipeline" / "bcb_plan2_kernel.py").read_text()
NSHARD = int(os.environ.get("NSHARD", "5"))
ROUND = os.environ.get("ROUND", "h50")
KDIR = ROOT / f"kernels_{ROUND}"
MAX_PER_ACCOUNT = 2

M15 = "xatri007/qwen2-5-1-5b-instruct"
M7  = "ragnar123/qwen2-5-7b-instruct"
DS  = {"mbpp": "mpwolke/mbppjsonl"}   # khong dung: BigCodeBench tai tu HF (internet BAT)
CELLS = [("mbpp", s) for s in ("15", "7")]
...
def main():
    ...
    for n, (task, size, sh) in enumerate(jobs):
        ...
        src = (TEMPLATE.replace("@@SHARD@@", str(sh)).replace("@@NSHARD@@", str(NSHARD))
                       .replace("@@SIZE@@", size).replace("@@RUN@@", "H50")
                       .replace("@@TIDLO@@", "11").replace("@@TIDHI@@", "510"))
        slug = f"library-composition-plan-{size}b-{sh}"
        ...
        meta = {"id": f"{user}/{slug}", "title": slug, "code_file": "kernel.py",
                ...
                "dataset_sources": [M15 if size == "15" else M7],
