...
# CHI cac tai khoan BAT duoc internet (autoawq/bitsandbytes khong co san tren anh Kaggle).
# 10 tai khoan con lai tra HTTP 400 khi enable_internet=true (chua xac minh dien thoai).
ALLOW = {'dfbjdsbds','dnglethnh','hduong','namphmbuwu','phmthanhlm24022379','tuananhtran37','tuetrandoanminh'}

ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = (ROOT / "pipeline" / "strat14_kernel.py").read_text()
NSHARD = int(os.environ.get("NSHARD", "20"))
ROUND = os.environ.get("ROUND", "h54b")
KDIR = ROOT / f"kernels_{ROUND}"
MAX_PER_ACCOUNT = 2

DATASETS = ["xatri007/qwen2-5-1-5b-instruct",
            "pjmathematician/qwen2514bawq",
            "open-benchmarks/math-500-measuring-mathematical-problem-solving"]

def main():
    accs = [a for a in t4_pool() if a[0] in ALLOW]
    ...
    for s in range(NSHARD):
        ...
        slug = f"math-scale14-shard-{s:02d}"
        d = KDIR / f"s{s:02d}"
        d.mkdir()
