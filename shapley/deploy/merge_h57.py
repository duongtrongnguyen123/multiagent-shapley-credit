#!/usr/bin/env python3
"""Gop H57 (dang ky truoc #62 (tai lap)): dung test tu sinh de CHON trong 8 mau.
Bao ca TI LE KHOANG TRONG THU DUOC = (select - maj8) / (oracle8 - maj8)."""
import sys, json, glob

RES = sys.argv[1] if len(sys.argv) > 1 else "res_h57"
...
for f in sorted(glob.glob(f"{RES}/**/res_H*s*.json", recursive=True)):
    ...
    if not str(d.get("tag","")).startswith("H57"): continue
...
if nfile < NEED: print(f"THIEU shard -> CHUA du de doc bang khoa #62 (tai lap)")
...
json.dump({"n": n, "avg_tests": round(avg_t,2), "test_soundness": round(sound,4),
           ...
           "gap_captured": round(cap,4)}, open(f"{RES}/H57_merged.json","w"), indent=2)
print(f"\nda ghi {RES}/H57_merged.json")
