...
JOBS=["code_m15","code_m7"]; LOG=open("monitor_code.log","a")
...
for it in range(48):
    ...
    say(f"[{it}|{nf}/2] "+" ".join(f"{j}:{res[j][0]}" for j in JOBS))
    if nf==2:
        json.dump({j:{"status":res[j][0],"summary":res[j][1]} for j in JOBS},open("monitor_code_result.json","w"),indent=2)
