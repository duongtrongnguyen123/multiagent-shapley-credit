...
P, S, V, A = MASK
print(f"CONFIG {CONFIG_ID} MASK P={P}(7B) S={S} V={V} A={A} N={N_EVAL}", flush=True)
...
if P:
    plans = gen([chat(PLANNER_DIR, PLAN_SYS, q) for q in questions], 256, PLANNER_DIR, 8)
if S:
    sol = gen([chat(SMALL_DIR, SOLVE_SYS, solve_user(i)) for i in range(n)], 512, SMALL_DIR, 32)
    for i, t in enumerate(sol):
        produced[i].append(t)
if V:
    prompts = [chat(SMALL_DIR, VERIFY_SYS, questions[i] + "\n\nProposed solution:\n" + produced[i][-1])
               if produced[i] else chat(SMALL_DIR, SOLVE_SYS, solve_user(i)) for i in range(n)]
    ver = gen(prompts, 512, SMALL_DIR, 32)
...
if A:
    ...
    for i in range(n):
        if produced[i]:
            ...
            prompts.append(chat(SMALL_DIR, AGG_SYS, questions[i] + "\n\n" + body))
        else:
            prompts.append(chat(SMALL_DIR, SOLVE_SYS, solve_user(i)))
    agg = gen(prompts, 256, SMALL_DIR, 32)
...
summary = {"config_id": CONFIG_ID, "mask": {"P": P, "S": S, "V": V, "A": A},
           "planner_model": "7B", "n": n, "correct": correct, "accuracy": acc,
